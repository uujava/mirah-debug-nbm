...
//        final MirahIndex index = MirahIndex.get(QuerySupport.findRoots(fo,
//                Collections.singleton(ClassPath.SOURCE), null, null));

        final MirahIndex index = MirahIndex.get(fo);
        ...
        for (IndexedClass indexedClass : classes) {
            ...
            String pkgName = MirahUtils.stripClassName(indexedClass.getFqn());
    ...
    private static Set<ImportCandidate> findJavaImportCandidates(FileObject fo, String packageName, String missingClass) {
        ...
        for (ElementHandle<TypeElement> typeName : typeNames) {
            ...
            // Skip classes within the same package
            String pkgName = MirahUtils.stripClassName(typeName.getQualifiedName());
...
*/        
    ...
    private static Map<String, Integer> collectImports(BaseDocument doc) throws BadLocationException {
        TokenSequence<MirahTokenId> ts = LexUtilities.getMirahTokenSequence(doc, 1);

        Map<String, Integer> result = new TreeMap<>();
        while (ts.moveNext()) {
            if (ts.token().id().is(Tokens.tImport)) {
                StringBuilder sb = new StringBuilder();

                IMPORT_COUNTER:
                while (ts.moveNext()) {
                    MirahTokenId tokenID = ts.token().id();
                    if (tokenID.is(Tokens.tIDENTIFIER)
...
//                        case DOT:
                    ...
                    if (tokenID.is(Tokens.tWhitespace))
                    {
//                        case WHITESPACE:
...
//                            break IMPORT_COUNTER;
    ...
    private static int getLastPackageStatementOffset(BaseDocument doc) {
        ...
        while (ts.moveNext()) {
            if (ts.token().id().is(Tokens.tPackage)) {

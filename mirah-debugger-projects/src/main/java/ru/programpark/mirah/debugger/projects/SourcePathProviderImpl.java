...
//        if (verbose)
//            LOG.info("SPPI: init originalSourcePath " + originalSourcePath);
//        if (verbose)
//            LOG.info(
...
//        if ( LOG != null ) LOG.info("SPPI: getURL " + relativePath + " global " + global);
        FileObject fo;
...
//                    if ( LOG != null ) LOG.info("SPPI:   fo " + fo);
                } else {
...
//                LOG.info("SPPI:   fo " + fo);
        ...
        try {
            ...
            if (verbose) System.out.println ("SPPI:   FileStateInvalidException");
            return null;
        }
    }
    ...
    public String[] getAllURLs (String relativePath, boolean global) {      
        ...
        for (FileObject fo : fos) {
            try {
                ...
                if (verbose) System.out.println ("SPPI:   FileStateInvalidException for "+fo);
                // skip it
            }
        }
...
//        LOG.info("getRelativePath url = "+url);
...
//        if ( LOG != null ) LOG.info("getSourceRoot url="+url);            
        FileObject fo;
        try {
...
//            LOG.fine("SourcePathProviderImpl.setSourceRoots("+java.util.Arrays.asList(sourceRoots)+")");
//        }
//    LOG.info("SourcePathProviderImpl.setSourceRoots(" + java.util.Arrays.asList(sourceRoots) + ")");        
    ...
    private static String getRoot(FileObject fileObject) {
        File f = null;
//        if ( LOG != null ) LOG.info("getRoot fileObject="+fileObject);            
        String path = "";
        try {
...
//            LOG.info("propertyChange evt="+evt);
            // JDK sources changed
            JavaPlatform[] platforms = JavaPlatformManager.getDefault ().
                getInstalledPlatforms ();

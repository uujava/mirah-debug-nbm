...
//        LOG.info("shouldBeEnabled Project = "+p);
        if (p == null) return false;
        ActionProvider actionProvider = (ActionProvider) p.getLookup ().
            lookup (ActionProvider.class);
//        LOG.info("shouldBeEnabled actionProvider = "+actionProvider);
        if (actionProvider == null) return false;
        String[] sa = actionProvider.getSupportedActions ();
        int i, k = sa.length;
        for (i = 0; i < k; i++)
        {
//            LOG.info("shouldBeEnabled getSupportedActions["+i+"] = "+sa[i]);
            if (ActionProvider.COMMAND_DEBUG_STEP_INTO.equals (sa [i]))
                break;
        }
        if (i == k) return false;

//        LOG.info("shouldBeEnabled DebuggerManager.getDebuggerManager().getDebuggerEngines().length = "+DebuggerManager.getDebuggerManager().getDebuggerEngines().length);
...
//        
//        LOG.info("propertyChange PropertyName = "+e.getPropertyName());
//        LOG.info("propertyChange Source = "+e.getSource());
//        LOG.info("propertyChange NewValue = "+e.getNewValue());
//        LOG.info("propertyChange OldValue = "+e.getOldValue());
//        LOG.info("propertyChange PropagationId = "+e.getPropagationId());
